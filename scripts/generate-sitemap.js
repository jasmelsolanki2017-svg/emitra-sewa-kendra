...
.ca-print-brand{display:none}.ca-hero{text-align:center;margin-bottom:20px}.ca-hero h1{font-size:40px;line-height:1.15;color:#072b61;font-weight:900}.ca-hero .date{display:block;color:#c5161d;font-size:36px}.ca-hero p{font-size:16px;margin-top:8px}
...
const renderStaticPostHtml = (id = "", job = {}, allRows = []) => {
  ...
  const { jsonLd: _legacyJsonLd, ...staticJob } = job || {};
  const staticPayload = `<script>window.__EMITRA_STATIC_POST__=${JSON.stringify({ id, job: { ...staticJob, slug: seo.slug, canonicalUrl } }).replace(/</g, "\\u003c")};</script>`;
...
const renderPremiumStaticPostHtml = (id = "", job = {}) => {
  const seo = buildSeoFields(job, id);
  const canonicalUrl = seo.canonicalUrl || jobUrl(id, { ...job, slug: seo.slug });
  const { jsonLd: _legacyJsonLd, ...cleanJob } = job || {};
  const payload = {
    ...cleanJob,
    slug: seo.slug,
    canonicalUrl,
